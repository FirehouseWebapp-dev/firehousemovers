...
class Gift_card(models.Model):
    ...
    added_by = models.ForeignKey(UserProfile,on_delete=models.CASCADE,related_name="added_by_user")

    def clean(self):
        if self.amount:
            if self.amount <1:
                raise ValidationError(f"The Gift Card amount cannot be less then one ({self.amount}).")
    ...
    def clean(self):
        ...
        if self.amount and self.card:
            if self.amount > self.card.amount:
                raise ValidationError(f"The award amount cannot exceed the amount of the associated gift card ({self.card.amount}).")
