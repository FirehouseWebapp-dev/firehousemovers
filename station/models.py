...
class Station_inspection(models.Model):
    station = models.ForeignKey(
        Station, related_name="inspections", on_delete=models.CASCADE
    )
    ...
    back_lot_cleanliness = models.CharField(
        max_length=40, choices=BACKLOT_CLEANLINESS_CHOICES, blank=True, null=True
    )
    back_lot_maintenance = models.CharField(
        max_length=40, choices=MAINTENANCE_CHOICES, blank=True, null=True
    )
    front_yard_cleanliness = models.CharField(
        max_length=40, choices=FRONT_CLEANLINESS_CHOICES, blank=True, null=True
    )
    front_yard_landscaping = models.CharField(
        max_length=40, choices=LANDSCAPING_CHOICES, blank=True, null=True
    )
    inventory_status = models.CharField(
        max_length=40, choices=INVENTORY_CHOICES, blank=True, null=True
    )
    missing_tools = models.CharField(max_length=255, blank=True, null=True)
    tool_cleanliness = models.CharField(
        max_length=40, choices=TOOL_CLEANLINESS_CHOICES, blank=True, null=True
    )
    tool_functionality = models.CharField(
        max_length=40, choices=FUNCTIONALITY_CHOICES, blank=True, null=True
    )
    bathroom_cleanliness = models.CharField(
        max_length=40, choices=BATH_CLEANLINESS_CHOICES, blank=True, null=True
    )
    bathroom_maintenance = models.CharField(
        max_length=40, choices=MAINTENANCE_CHOICES, blank=True, null=True
    )
    supplies = models.CharField(
        max_length=40, choices=SUPPLIES_CHOICES, blank=True, null=True
    )
    extinguisher_status = models.CharField(
        max_length=40, choices=EXTINGUISHER_CHOICES, blank=True, null=True
    )
    emergency_exit_status = models.CharField(
        max_length=40, choices=EMERGENCY_CHOICES, blank=True, null=True
    )
...
class Vehicle_inspection(models.Model):
    ...
    station = models.ForeignKey(
        Station, related_name="vehicle_inspections", on_delete=models.CASCADE
    )
    type = models.CharField(
        max_length=40, choices=INSPECTION_TYPE_CHOICES, null=True, blank=True
    )
    ...
    def __str__(self):
        return (
            f"{self.type.capitalize()} Service - {self.vehicle.number} on {self.date}"
        )
...
class Fleet_order(models.Model):
    ...
    station = models.ForeignKey(
        Station, related_name="station_orders", on_delete=models.CASCADE
    )
    type = models.CharField(
        max_length=40, choices=ORDER_TYPE_CHOICES, null=True, blank=True
    )
    urgency_level = models.CharField(
        max_length=40, choices=URGENCY_LEVEL_CHOICES, null=True, blank=True
    )
