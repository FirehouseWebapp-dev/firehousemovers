...
class UniformAssignment(models.Model):
    ...
    uniform = models.ForeignKey(
        UniformCatalog, on_delete=models.CASCADE, null=True, blank=True
    )
    quantity = models.PositiveIntegerField(null=True, blank=True)
    condition = models.CharField(
        max_length=10, choices=[("New", "New"), ("Used", "Used")], null=True, blank=True
    )
...
class Inventory(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        self.total_stock = (
            (self.new_stock if self.new_stock is not None else 0)
