...
class Add_uniform_view(View):
    ...
    def get(self,request):
        ...
        return render(request, "add_uniform.html", {
            "form": form,
        })
    ...
    if employee_id:
        try:
            ...
            for uniform in all_uniforms:
                ...
                if uniform_id not in latest_uniforms or latest_uniforms[uniform_id].id < uniform.id:
...
class Issue_uniform_view(View):
    ...
    def get(self,request):
        ...
        return render(request, "issue_uniform.html", {
            "form": form,
        })
...
class Employee_view(View):
    ...
    def post(self, request):
        action = request.POST.get('action')  # This checks whether we're adding or deleting
...
class inventory_view(View):
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            in_use_uniforms=UniformAssignment.objects.filter(uniform=uniform,status='Active').count()
            ...
            if 'inventory-remove' in request.path:
                ...
                if condition == "New":
                    ...
                    inventory.used_stock = max((inventory.used_stock or 0) - quantity, 0)
                if transaction_type=='Dispose':
                    inventory.disposed = (inventory.disposed or 0) + quantity 

                else:
                    inventory.return_to_supplier = (inventory.return_to_supplier or 0) + quantity
                    inventory.total_bought = max((inventory.total_bought or 0) - quantity, 0)
...
class Reports_view(View):
    ...
    def post(self,request):
        ...
        if inventory_summary=='inventory_summary':
            ...
            return render(request, "reports.html", {"inventory_records":inventory_records})
        else:
